...
function OngoingBatch({ batches, onEdit, onDelete }) {
  ...
  const handleManageClick = () => {
    handleMenuClose();
    // Navigate to '/managebatch' with the selected batch data
    navigate('/managebatch', { state: { selectedBatch } });
  };

  return (
    <>
      {batches.length === 0 ? (
        <div className="no-batches-message">No Ongoing Batches available</div>
      ) : (
              ...
              <tr key={batch.batchId}>
                ...
                <td>
                  ...
                  <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={handleMenuClose}>
                    ...
                    <MenuItem className='MenuItem' onClick={handleManageClick}><SettingsOutlinedIcon />Manage</MenuItem>
