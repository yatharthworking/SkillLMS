...
import { renderStars } from "../User/UserSidebarSections/UserStarRating.js";
...
export default function PublicCourse() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        const response = await axios.get(`${BACKEND_BASEURL}/public/getCourses`);
        const data = response.data;
        
        // Use classroomMaterials for classroom tab
        // Use certificationCourses for skill tab
        if (selectedSection === 'Classroom') {
          setCourses(data.classroomMaterials);
        } else if (selectedSection === 'Skill') {
          setCourses(data.certificationCourses);
        }
      } catch (error) {
    ...
    <div className="coursePageView">
      <header className="landingPageHeader">
        ...
        <div className="landingPageNav">
          <div className="navPointer" onClick={handleCourseClick} style={{color:'#FFB703'}}>
            Courses
          </div>
      ...
      <div className='fullCourseDetails'>
      <div className="backgroundImageHeader">
        <div className="landingCourseHeaderSecondary">Courses</div>
      </div>
      <div className="landingCoursePageSection">
        <div className="courseSubHeaderSection">
          {/* Toggle Section */}
              ...
              <div className="userHeaderFilterSection">
                <div className="userSearchBox">
                  <input
                    ...
                    placeholder="Search a course ,teacher, etc..."
        ...
        <div className="publicCourseDetails">
          ...
          {loading ? (
            <div className="loadingContainer" style={{height:'auto'}}>
                    <BeatLoader color={"#219EBC"} loading={loading} size={15}  />
                  </div>
          ) : (
            courses?.length > 0 ? (
              courses?.map(course => (
            <div className="userCourseBox" key={course?.materialId}>
              ...
              <div className="userCourseBoxTxtSection">
                <div className="userCourseBoxTxtContainer">
                  <div className="userCourseBoxHeader">{course?.materialName}</div>
                  <div className="userCourseBoxSubHeader">By {course?.tutorName}</div>
                </div>

                <div className="userCourseBoxTxtContainer">
                  <div className="userCourseBoxRatingSection">
                    <span className="ratingNumberSection">4</span>
                    <span className="ratingstarSection">{renderStars(4)}</span>
                    <span className="ratingCountSection">(12)</span>
                  </div>
              ...
              <div className="userCourseBoxBtnSection">
                <button className="viewDetailBtn" onClick={handleLoginClick}>View Details</button>
                <button className="enrollBtn" onClick={handleLoginClick}>Purchase</button>
              </div>
            </div>
          ))
        ) : (
                 ...
                 <div className='noDataImage'>
                      ...
                      <div style={{ fontWeight: "600",fontSize:'20px' }}> No courses available...</div>
                </div>
        )
      )}
